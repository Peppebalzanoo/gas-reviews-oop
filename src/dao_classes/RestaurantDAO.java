...
public class RestaurantDAO {
	...
	public ArrayList<String> getRestaurantTypes(Integer restaurant_reference) throws OperationFailedException{
		
		String query = "select title from restaurant_type where restaurant_reference = ?";

		try {
	
			PreparedStatement preparedStmt = conn.prepareStatement(query);
	   
			preparedStmt.setInt(1, restaurant_reference);
	...
	public ArrayList<Restaurant> getRestaurantsByFilters (String title_filter , String country_filter, String type_filter, String address_filter, String price_filter, String rating_average_filter) throws OperationFailedException {
		...
		String query = "select st.id, st.title, st.country, st.address, st.price, st.rating_average, st.picture, rs.id, rs.seats, rs.rooms, rs.structure_reference "
				+ "from public.structure as st inner join public.restaurant as rs on st.id = rs.structure_reference "
		...
		try {
				...
				
			{
				...
				do{
					...
					ArrayList<String> typesList = this.getRestaurantTypes(rs.getInt(8));

					Restaurant s = new Restaurant(rs.getInt(1),
											rs.getString(2), 
											rs.getString(3), 
											rs.getString(4), 
											rs.getString(5), 
					...
					if(typesList.contains(type_filter) ||  isNotTypeFilterSetted ) restaurantsList.add(s);
